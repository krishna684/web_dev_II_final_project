  ...
  return (
    <div className="page">
      <div className="dash-hero">
        ...
        <p>You currently have <b style={{color:'var(--text)'}}>{personas.length} active sides</b> and <b style={{color:'var(--text)'}}>{matches.length} new compatibility matches</b> waiting.</p>
        <div className="hero-actions">
          <button className="btn btn-primary" onClick={()=>goTo('personas-new')}><Ic.plus/> Add a New Side</button>
        </div>
      </div>

      <div className="stat-row" style={{gridTemplateColumns: 'repeat(3, 1fr)'}}>
        <StatCard label="Sides created" value={personas.length} delta="+1 this month"/>
        <StatCard label="Top match score" value={`${Math.max(...matches.map(m=>m.score))}%`} delta="+6 vs last week"/>
        <StatCard label="Matches found" value="128" delta="+18"/>
      </div>
      ...
      <div className="dash-split">
              ...
              <div key={p.id} className="persona-card" onClick={()=>openPersona(p.id)}>
                ...
                <div className="score-row">
                  ...
                  <div className="actions">
                    <button className="btn btn-sm btn-ghost" onClick={(e)=>{e.stopPropagation(); openPersona(p.id);}}>Edit</button>
        ...
        <div>
          ...
          <div className="section-head" style={{marginTop:24}}>
            <h3>Connections</h3>
            <span className="hint">Visual</span>
          </div>
